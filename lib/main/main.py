...
def display_results(accuracy, report, cm, feature_importance, X_test, y_test, y_pred):
    ...
    display(HTML("<h3 style='font-family:Arial; color:#333;'>Relatório de Classificação</h3>"))
    display(styled_df)
    
    fig, ax = plt.subplots(2, 2, figsize=(14, 10))
    
    sns.histplot(X_test['amount'][y_test == 0], color='blue', alpha=0.5, label='Não Fraude', ax=ax[0, 0], kde=True)
    sns.histplot(X_test['amount'][y_test == 1], color='red', alpha=0.5, label='Fraude', ax=ax[0, 0], kde=True)
    ax[0, 0].set_title('Distribuição de Valores')
    ax[0, 0].set_xlabel('Valor da Transação')
    ax[0, 0].legend()
    
    sns.histplot(X_test['customer_age'][y_test == 0], color='blue', alpha=0.5, label='Não Fraude', ax=ax[0, 1], kde=True)
    sns.histplot(X_test['customer_age'][y_test == 1], color='red', alpha=0.5, label='Fraude', ax=ax[0, 1], kde=True)
    ax[0, 1].set_title('Distribuição de Idades')
    ax[0, 1].set_xlabel('Idade do Cliente')
    ax[0, 1].legend()
    
    scatter = ax[1, 1].scatter(X_test['amount'], X_test['time'], c=y_pred, cmap='coolwarm', alpha=0.6)
    ax[1, 1].set_title('Detecção de Fraude: Valor vs Horário')
...
def check_and_install_deps():
    try:
        ...
        !pip install -q ipywidgets seaborn matplotlib
